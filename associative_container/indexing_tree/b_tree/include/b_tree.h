...
private:
    ...
    {
        if (n) _allocator.template delete_object<btree_node>(n);
    }
    ...
    void destroy_subtree(btree_node* n)
    {
        if (!n) return;
        if (!is_leaf(n))
            for (auto* c : n->_pointers) destroy_subtree(c);
        delete_node(n);
    }
    btree_node* copy_subtree(const btree_node* src)
    {
        ...
        try
        {
            for (auto const& kv : src->_keys) n->_keys.push_back(kv);
            if (!is_leaf(src))
                for (auto* c : src->_pointers) n->_pointers.push_back(copy_subtree(c));
        }
        catch (...)
        {
            if (!is_leaf(n))
    ...
    size_t lower_in_node(const btree_node* n, const tkey& key) const
    {
        ...
        {
            size_t mid = (lo + hi) / 2;
            if (compare_keys(n->_keys[mid].first, key)) lo = mid + 1;
            else hi = mid;
        }
        return lo;
    }
    ...
    std::pair<btree_node*, tree_data_type> split_overfull(btree_node* y)
    {
        ...
        y->_keys.resize(med);
        if (!is_leaf(y)) y->_pointers.resize(med + 1);
    ...
    bool insert_post(K&& key, V&& val)
    {
        ...
        while (true)
        {
            ...
            if (is_leaf(cur))
            {
                cur->_keys.insert(cur->_keys.begin() + i,
    ...
    void ensure_can_descend(btree_node* parent, size_t i)
    {
        ...
        if (left)
        {
            ...
            if (!is_leaf(child))
                for (auto* p : child->_pointers) left->_pointers.push_back(p);
        ...
        {
            child->_keys.push_back(std::move(parent->_keys[i]));
            for (auto& kv : right->_keys) child->_keys.push_back(std::move(kv));
            if (!is_leaf(right))
                for (auto* p : right->_pointers) child->_pointers.push_back(p);
    ...
    bool erase_from(btree_node* n, const tkey& key)
    {
        ...
        if (here)
        {
            ...
            {
                btree_node* cur = rc;
                while (!is_leaf(cur)) cur = cur->_pointers.front();
            ...
            if (!is_leaf(rc))
                for (auto* p : rc->_pointers) lc->_pointers.push_back(p);
            n->_keys.erase(n->_keys.begin() + i);
...
public:
    ...
    explicit B_tree(iterator begin, iterator end, const compare& cmp = compare(), pp_allocator<value_type> alloc = pp_allocator<value_type>())
        : compare(cmp), _allocator(alloc), _root(nullptr), _size(0)
    {
        for (auto it = begin; it != end; ++it) emplace(it->first, it->second);
    }
    ...
    B_tree(std::initializer_list<std::pair<tkey, tvalue>> data, const compare& cmp = compare(), pp_allocator<value_type> alloc = pp_allocator<value_type>())
        : compare(cmp), _allocator(alloc), _root(nullptr), _size(0)
    {
        for (auto const& kv : data) emplace(kv.first, kv.second);
    }
    ...
    B_tree(B_tree&& other) noexcept
        : compare(std::move(static_cast<compare&>(other))), _allocator(std::move(other._allocator)),
    ...
    public:
        ...
        self& operator++()
        {
            ...
            {
                _path.pop();
                if (_path.empty()) { _index = 0; return *this; }
    ...
    public:
        ...
        explicit btree_const_iterator(const std::stack<std::pair<btree_node* const*, size_t>>& path = {}, size_t index = 0)
            : _path(path), _index(index) {}
        ...
        btree_const_iterator(const btree_iterator& it) noexcept : _index(it._index)
        {
            ...
            while (!tmp.empty()) { buf.push_back(tmp.top()); tmp.pop(); }
        ...
        self& operator++()
        {
            ...
            if (_index + 1 < node->_keys.size()) { ++_index; return *this; }
            while (true)
            {
                _path.pop();
                if (_path.empty()) { _index = 0; return *this; }
        ...
        self& operator--()
        {
            ...
            while (true)
            {
                ...
                if (_path.empty()) { _index = 0; return *this; }
                size_t came_from = _path.top().second;
                if (came_from > 0) { _index = came_from - 1; return *this; }
            }
        }
        self operator--(int) { auto tmp = *this; --(*this); return tmp; }
    };
    ...
    public:
        ...
        explicit btree_reverse_iterator(const std::stack<std::pair<btree_node**, size_t>>& path = {}, size_t index = 0)
            : _it(path, index) {}
        btree_reverse_iterator(const btree_iterator& it) noexcept : _it(it) {}
        operator btree_iterator() const noexcept { auto tmp = _it; --tmp; return tmp; }

        reference operator*() const noexcept { auto tmp = _it; --tmp; return *tmp; }
        pointer operator->() const noexcept { auto tmp = _it; --tmp; return tmp.operator->(); }

        self& operator++() { --_it; return *this; }
        self operator++(int) { auto tmp = *this; --_it; return tmp; }
        self& operator--() { ++_it; return *this; }
        self operator--(int) { auto tmp = *this; ++_it; return tmp; }

        bool operator==(const self& o) const noexcept { return _it == o._it; }
        bool operator!=(const self& o) const noexcept { return _it != o._it; }

        size_t depth() const noexcept { return _it.depth(); }
        ...
        bool is_terminate_node() const noexcept { return _it.is_terminate_node(); }
    };
    ...
    public:
        ...
        explicit btree_const_reverse_iterator(const std::stack<std::pair<btree_node* const*, size_t>>& path = {}, size_t index = 0)
            : _it(path, index) {}
        btree_const_reverse_iterator(const btree_reverse_iterator& it) noexcept : _it(static_cast<btree_iterator>(it)) {}
        operator btree_const_iterator() const noexcept { auto tmp = _it; --tmp; return tmp; }
    ...
    btree_reverse_iterator rbegin() { auto e = end(); return btree_reverse_iterator(e); }
    ...
    btree_const_reverse_iterator rbegin() const { return crbegin(); }
    btree_const_reverse_iterator rend() const { return crend(); }
    ...
    {
        auto it = find(key);
        if (it == cend()) throw std::out_of_range("B_tree::at: key not found");
        return it->second;
    }
